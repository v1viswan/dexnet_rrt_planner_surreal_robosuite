...
class FixedPositionSampler(ObjectPositionSampler):
    """Places all objects from a list of positions provided"""
    ...
    def __init__(
        ...
        """
        Args:
            pos_list: A list of positions(x,y and z rotation). These positions must be given considering
                    the center of table arena as the origin and x-y plane being the table arena
        """
    ...
    def sample(self):
        ...
        for obj_mjcf in self.mujoco_objects:
            ...
            quat = [np.cos(self.pos_list[index][2] / 2), 0, 0, np.sin(self.pos_list[index][2] / 2)]
